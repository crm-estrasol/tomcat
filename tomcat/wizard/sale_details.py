...
class SaleDiscount(models.TransientModel):
    ...
    def generate_report(self):
        for prod in self.sale.order_line:
            prod.discount = self.discount  if prod.project in self.projects or prod.product_id.brand in self.brand else prod.discount or prod.ubication  in self.ubications
        return self.sale
    @api.onchange('sale')
